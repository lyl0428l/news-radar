...
def _ensure_browser():
    """确保浏览器实例可用（线程安全，懒加载）— 外部调用版"""
    with _lock:
        return _ensure_browser_unsafe()


def _ensure_browser_unsafe():
    """确保浏览器实例可用（调用者已持锁）"""
...
def fetch_page_html(url: str, timeout: int = 15,
    ...
    **线程安全**：整个函数用锁保护，同一时刻只有一个线程使用 Playwright。
    Playwright 的 sync API 内部依赖 asyncio 事件循环，多线程并发会冲突崩溃。
    ...
    """
    # 整个操作加锁，防止多线程并发导致 asyncio 事件循环冲突
    with _lock:
        browser = _ensure_browser_unsafe()
        if browser is None:
            return ""
